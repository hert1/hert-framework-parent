 ...
 */
package com.hert.core.tool.jackson;
...
public class MappingApiJackson2HttpMessageConverter extends AbstractReadWriteJackson2HttpMessageConverter {
	...
	private static ObjectMapper initWriteObjectMapper(ObjectMapper readObjectMapper) {
		...
		// null 处理
		writeObjectMapper.setSerializerFactory(writeObjectMapper.getSerializerFactory().withSerializerModifier(new HertBeanSerializerModifier()));
		writeObjectMapper.getSerializerProvider().setNullValueSerializer(HertBeanSerializerModifier.NullJsonSerializers.STRING_JSON_SERIALIZER);
